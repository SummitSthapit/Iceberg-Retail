...
def main():
    spark=create_spark("DQ_GoldSalesAmount"); spark.sparkContext.setLogLevel("WARN"); failures=[]
    for name,table in TABLES.items():
        n=spark.table(table).filter(col("total_sales")<0).count(); print(f"{table} negative total_sales records: {n}")
        if n>0: failures.append(f"{table}: {n} negative total_sales records")
    if failures: raise RuntimeError(f"DATA QUALITY FAILED: Negative total_sales values found: {failures}")
    print("DATA QUALITY PASSED: All Gold sales amounts are non-negative"); spark.stop()
