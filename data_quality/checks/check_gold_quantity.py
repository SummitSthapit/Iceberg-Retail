...
def main():
    spark=create_spark("DQ_GoldQuantity"); spark.sparkContext.setLogLevel("WARN"); failures=[]
    for name,table in TABLES.items():
        n=spark.table(table).filter(col("total_quantity")<=0).count(); print(f"{table} invalid total_quantity records: {n}")
        if n>0: failures.append(f"{table}: {n} invalid total_quantity records")
    if failures: raise RuntimeError(f"DATA QUALITY FAILED: total_quantity must be greater than zero: {failures}")
    print("DATA QUALITY PASSED: All Gold total quantities are greater than zero"); spark.stop()
